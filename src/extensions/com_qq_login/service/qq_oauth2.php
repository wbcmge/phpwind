...
class qq_oauth2{
	 ...
	 * @return int uid
	 */
	function register_user($mail,$nick,&$error=false){
		!session_id() && session_start();
		...
		//检查用户名
		$result = PwUserValidator::isUsernameValid($nick);
		if ($result instanceof PwError){
			$error = $result->getError(false);
			return;
		}
		
		//检查邮箱
		$result = PwUserValidator::isEmailValid($mail, $nick);
		if ($result instanceof PwError){
			$error =  $result->getError(false);
			return;
		}
	 ...
	 */
	function _UserDM($mail,$nick){
		Wind::import('SRC:service.user.dm.PwUserInfoDm');
		$userDm = new PwUserInfoDm();
		$password = substr(md5(rand().$mail.$nick),0,15);
		$userDm->setUsername($nick);
		$userDm->setPassword($password);
		$userDm->setEmail($mail);
